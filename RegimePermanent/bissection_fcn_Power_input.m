function [v_final,tau_final] = bissection_fcn_Power_input(vA,vB,precision,Pinput,intervalle_tau)
...
% l'algorithme converge.
 ...
 yvA =  get_tau_e_v_devine(vA,intervalle_tau,Pinput);
%  get_tau_e_v_devine ; Il s'agit de la fonction à mettre = 0 à l'aide de
%  la méthode de la bissection.
 yvB =  get_tau_e_v_devine(vB,intervalle_tau,Pinput);
while abs(vA-vB) > precision
    vC=(vA+vB)/2;
%     [yC,~,~,~,~,cC,DfC] =  fcn_hdle(v,C);
yC =  get_tau_e_v_devine(vC,intervalle_tau,Pinput);
    if (yvA*yC<=0)
        vB=vC;
    else
        vA=vC;
...
[~,tau_final] =  get_tau_e_v_devine(v_final,intervalle_tau,Pinput);
end
%%
